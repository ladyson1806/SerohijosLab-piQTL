...
def get_string_network(root_path):
    string_network = pd.read_csv(f'{root_path}networks/STRING/4932.protein.physical.links.detailed.v11.5.txt', sep=' ')
    ...
    final_string_network = string_network[string_network['experimental'] > 0.5].reset_index(drop=True)
    return final_string_network[['protein1', 'protein2', 'experimental']]


def get_biogrid_network(root_path, exp_type):
    biogrid_network = pd.read_table(f'{root_path}networks/BioGrid/BIOGRID-ORGANISM-4.4.200.tab3/BIOGRID-ORGANISM-Saccharomyces_cerevisiae_S288c-4.4.200.tab3.txt', low_memory=False)
    biogrid_network = biogrid_network[(biogrid_network['Organism Name Interactor A'] == 'Saccharomyces cerevisiae (S288c)' ) & (biogrid_network['Organism Name Interactor B'] == 'Saccharomyces cerevisiae (S288c)' )]
    biogrid_network_exp = biogrid_network[biogrid_network['Experimental System'] == exp_type].reset_index(drop=True).rename(columns={'Systematic Name Interactor A':'protein1', 'Systematic Name Interactor B':'protein2'})
...
def get_tarassov_network(root_path, protein_abundance):
    index_ids = pd.read_csv(f'{root_path}networks/tarassov_updated_dataset/alpha_array.csv', names=['index'])['index']
    columns_ids = pd.read_csv(f'{root_path}networks/tarassov_updated_dataset/allInteractionsNames.csv', names=['columns'])['columns']
    intensity_matrix = pd.read_table(f'{root_path}networks/tarassov_updated_dataset/allInteractionsIntensities.txt', header=None)
    ...
    intensity_matrix = intensity_matrix[intensity_matrix.index.isin(true_idx_pa)][true_col_pa]
    print(intensity_matrix.shape)
    
    tarassov_edges = get_tarassov_intensity(intensity_matrix)
    
    tarassov_network = pd.DataFrame(tarassov_edges, columns=['protein1', 'protein2', 'intensity'])
...
def get_cellmap_network(root_path):
    cellMap_similarity_matrix = pd.read_table(f'{root_path}/networks/cellMap/similarity_matrices/cc_ALL.txt', sep='\t', low_memory=False)
