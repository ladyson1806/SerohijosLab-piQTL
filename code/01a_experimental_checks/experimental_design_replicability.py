...
def fitness_comparison(PPI, DRUG, DOUBLE):
    ...
    for MTX in ['noMTX', 'MTX']:
        ...
        sns.scatterplot(data=REP_MERGED, x='logratio_Fitness_A', y='logratio_Fitness_B', linewidth=0, color='black', ax=axes[i])
        axes[i].set_xlabel('$Fitness_{~Replicate~A}$')
        axes[i].set_ylabel('$Fitness_{~Replicate~B}$')
        sns.scatterplot(data=REP_MERGED[REP_MERGED['strain_id'].str.contains('_ref')], x='logratio_Fitness_A', y='logratio_Fitness_B', linewidth=0, color='red', ax=axes[i])
        ...
        axes[i].annotate(f"Pearson's corr: {round(corr,3)}\npval: {'{:.2e}'.format(pval)}", xy=[0, -55], xycoords="axes points")
...
def display_density_plot(all_A_vs_B, MTX, SINGLE):
    ...
    fig = plt.figure()
    fig, ax = using_mpl_scatter_density(fig, all_A_vs_B['logratio_Fitness_A'], all_A_vs_B['logratio_Fitness_B'])
    ax.set_xlabel('$Fitness_{~Replicate~A~}$')
    ax.set_ylabel('$Fitness_{~Replicate~B~}$')
    corr, pval = stats.pearsonr(all_A_vs_B['logratio_Fitness_A'], all_A_vs_B['logratio_Fitness_B'])
    ax.set_xlim([-5, 20])
    ax.set_ylim([-5, 20])
    ax.set_box_aspect(1)
    ax.annotate(f"Pearson's score: {round(corr,3)}\npval: {'{:.2e}'.format(pval)}", xy=[0, 0], xycoords="figure points")
    plt.title(MTX)
    fig.savefig(f'../../figures/figure_1/replicability_density_plot_{MTX}.eps', dpi=300)
    # fig.savefig(f'../manuscript/figures/figure_1/replicability_density_plot_{MTX}.png', dpi=300)
...
def plot_number_strains_stat(logs, SINGLE) : 
    ...
    nb_strain_stats['REP'] = [ nb_strain_stats['Condition'][idx].split('_')[0] for idx in nb_strain_stats.index ]
    nb_strain_stats['MTX'] = [ nb_strain_stats['Condition'][idx].split('_')[1] for idx in nb_strain_stats.index ]
    nb_strain_stats['DRUG'] = [ nb_strain_stats['Condition'][idx].split('.')[0].split('_')[2] for idx in nb_strain_stats.index ]
    ...
    print('Average number of strains per sample', np.mean(nb_strain_stats['nb_strains']))

    print(len(nb_strain_stats[nb_strain_stats['nb_strains'] == 361])/len(nb_strain_stats))

    print(len(nb_strain_stats[(nb_strain_stats['nb_strains'] >= 325) & (nb_strain_stats['nb_strains'] <= 360)])/len(nb_strain_stats))
    
    sns.histplot(nb_strain_stats['nb_strains'], stat='percent', binwidth=10, binrange=(0,361), color='grey')
    plt.xticks([0, 60, 120, 180, 240, 300, 360], ["0", "60", "120", "180", "240", "300", "360"])
    plt.ylabel('Percentage (%)')
    plt.xlabel('Number of retrieved strains')
    f.savefig(f'../../figures/figure_1/strain_mapping_distribution.eps', dpi=300)
    # f.savefig(f'../manuscript/figures/figure_1/strain_mapping_distribution.png', dpi=300)
...
if __name__ == "__main__":
    ...
    logs = [ pd.read_csv(os.path.join(log_folder, log)) for log in os.listdir(log_folder)]
    plot_number_strains_stat(logs, SINGLE)



    # REP1 = MTX[MTX['strain_id'] == '17']
    # REP2 = MTX[MTX['strain_id'] == '17_2']
    # REP_17 = REP1.merge(REP2, on='MTX_condition', suffixes=['_REP1', '_REP2'])
    # REP_17['REP_ID'] = '17'

    # REP1 = MTX[MTX['strain_id'] == '40']
    # REP2 = MTX[MTX['strain_id'] == '40_2']
    # REP_40 = REP1.merge(REP2, on='MTX_condition', suffixes=['_REP1', '_REP2'])
    # REP_40['REP_ID'] = '40'

    # REP1 = MTX[MTX['strain_id'] == '180']
    # REP2 = MTX[MTX['strain_id'] == '180_2']
    # REP_180 = REP1.merge(REP2, on='MTX_condition', suffixes=['_REP1', '_REP2'])
    # REP_180['REP_ID'] = '180'

    # REP1 = MTX[MTX['strain_id'] == '43_ref1']
    # REP2 = MTX[MTX['strain_id'] == '43_ref2']
    # REP_43_REF = REP1.merge(REP2, on='MTX_condition', suffixes=['_REP1', '_REP2'])
    # REP_43_REF['REP_ID'] = '43_REF'

    # REP1 = MTX[MTX['strain_id'] == '599_ref1']
    # REP2 = MTX[MTX['strain_id'] == '599_ref2']
    ...
    # print(REPS)
    # sns.scatterplot(x=REPS['logratio_Fitness_A_REP1'], y=REPS['logratio_Fitness_A_REP2'], hue=REPS['REP_ID'])
    # corr, pval = stats.pearsonr(REPS['logratio_Fitness_A_REP1'], REPS['logratio_Fitness_A_REP2'])
    # print(corr, pval)
    # plt.show()
    
    # sns.scatterplot(x=REPS['logratio_Fitness_B_REP1'], y=REPS['logratio_Fitness_B_REP2'], hue=REPS['REP_ID'])
    # corr, pval = stats.pearsonr(REPS['logratio_Fitness_B_REP1'], REPS['logratio_Fitness_B_REP2'])
    # print(corr, pval)
    # plt.show()