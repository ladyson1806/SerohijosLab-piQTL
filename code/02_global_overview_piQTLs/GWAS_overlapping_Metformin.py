...
yeast_diabetes_II_homologs = human_homologs[
    human_homologs["Human_Gene_name"].isin(human_diabetesII_QTLs)
]["Yeast_Gene_stable_ID"]
yeast_diabetes_II_homologs_with_snps = set(yeast_diabetes_II_homologs) & set(
    ORF_with_SNPs
)

yeast_ES_piQTLs = [
    gene
    for gene in np.unique(ES_piQTLs[(ES_piQTLs["snps_class_up"] == "ORF")]["locus_id"])
]

yeast_NES_piQTLs = [
    gene
    for gene in np.unique(
        NES_piQTLs[(NES_piQTLs["snps_class_up"] == "ORF")]["locus_id"]
    )
]
yeast_all_piQTLs = [
    gene for gene in np.unique(piQTLs[(piQTLs["snps_class_up"] == "ORF")]["locus_id"])
]
...
out = venn3(
    [
        set(yeast_homologs_with_snps),
        set(yeast_ES_piQTLs_with_homologs),
        set(yeast_diabetes_II_homologs_with_snps),
    ],
    set_colors=("green", "white", "purple"),
    set_labels=[
        "ORFs with annotated SNPs",
        "Metformin \nORF piQTLs",
        "Yeast ORF homologs of human genes \nassociated to Type II Diabetes",
    ],
)
plt.show()
...
for locus_id in set(yeast_ES_piQTLs) & set(yeast_diabetes_II_homologs):
    print(locus_id)
    TMP = piQTLs[
        (piQTLs["locus_id"] == locus_id)
        & (piQTLs["Condition"].str.contains("Metformin"))
    ]
    if len(TMP) != 0:
        print(locus_id)
        res.append(
