...
ES_piQTLs = pd.read_csv(f'{root_path}/results/05_piQTL_tables/ES_vs_NES/Trifluoperazine_ES_piQTLs.csv')
NES_piQTLs = pd.read_csv(f'{root_path}/results/05_piQTL_tables/ES_vs_NES/Trifluoperazine_NES_piQTLs.csv')

#### Loading GWAS hits associated with schizophrenia
schizophrenia_gwas = pd.read_table(f'{root_path}/data/gwas_annotations/gwas_catalog_v1.0.2-associations_e107_r2022-10-08.tsv')
...
if not path.exists(f"{root_path}/figures/FIGURE_3"):
    makedirs(f"{root_path}/figures/FIGURE_3")
f.savefig(f'{root_path}/figures/FIGURE_3/FIGURE_3B.pdf', dpi=300)
f.savefig(f'{root_path}/figures/FIGURE_3/FIGURE_3B.eps', dpi=300)
f.savefig(f'{root_path}/figures/FIGURE_3/FIGURE_3B.png', dpi=300)
...
TABLE = pd.concat(res)
TABLE.to_csv(f'{root_path}/results/06_GWAS_overlapping/human_GWAS_vs_piQTLs_trifluoperazine_hits.csv', index=False)
TABLE = TABLE.merge(human_homologs, left_on='locus_id', right_on='Yeast_Gene_stable_ID')
TABLE.to_csv(f'{root_path}/results/06_GWAS_overlapping/human_GWAS_vs_piQTLs_trifluoperazine_hits_with_human_homologs.csv', index=False)
